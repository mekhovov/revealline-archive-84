...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (1149, 595699929))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.115.1'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-84/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], '24c1dea2a86c595d935c47857aca672cc2d2afd9')
        self.assertEqual(release['sourceRevision'], '6f13bd14496e3b1a6b929c78638b45ba07a7c753')
        self.assertEqual(release['sourceTree'], 'a63b46bcc8da3da08b270e74f433eb703dcbbfeb')
        self.assertEqual(release['sourceQualification'], {'bytes': 14729, 'sha256': 'f168bc582b312081150ed66b23ec6cb228e82cce837341cf7e444eb2dce5389f', 'policyEvidence': {'path': 'publishing/test-policy.json', 'bytes': 490, 'sha256': 'b6887ba7f2b84a007b96de14fc867ac38ac8135d7c231c7ade7ebc52ba31704a'}})
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.115.1/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (1146, 595698532))
